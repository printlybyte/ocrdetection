...
public class CameraActivity extends AppCompatActivity implements TabLayout.OnTabSelectedListener {
    ...
    private void addTabToTabLayout() {
        String[] mTitles = {"身份证(正)", "身份证(反)", "银行卡", "驾驶证", "行驶证", "车牌", "营业执照"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        outputPath = FileUtil.getSaveFile(getApplication()).getAbsolutePath();
        contentType = CameraActivity.CONTENT_TYPE_ID_CARD_FRONT;
...
//        takePictureContainer.setOrientation(orientation);
    ...
    public void onTabSelected(TabLayout.Tab tab) {
        ...
        Log.i("TESTRE", "" + tab.getPosition());
        outputPath = FileUtil.getSaveFile(getApplication()).getAbsolutePath();
        if (TextUtils.isEmpty(outputPath)) {
            Toast.makeText(this, "本地地址获取失败", Toast.LENGTH_SHORT).show();
            return;
        }

        if ((tab.getText().toString()).equals("身份证(正)")) {
            contentType = CameraActivity.CONTENT_TYPE_ID_CARD_FRONT;
        }
        if ((tab.getText().toString()).equals("身份证(反)")) {
            contentType = CameraActivity.CONTENT_TYPE_ID_CARD_BACK;
        }
        if ((tab.getText().toString()).equals("驾驶证")) {
            contentType = CameraActivity.CONTENT_TYPE_ID_CARD_BACK;
        }
            Log.i("TESTRE", "" + tab.getText().toString());
...
//        Log.i("TESTRE", "onTabUnselected" + tab.getPosition());
    }

    @Override
    public void onTabReselected(TabLayout.Tab tab) {
//        Log.i("TESTRE", "onTabReselected" + tab.getPosition());
    }
}
